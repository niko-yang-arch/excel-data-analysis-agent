...
每张图表由系统按数据形态自动生成：少量非负的分类构成用饼图，分类对比用柱状图，时间序列用折线图，两列数值的成对关系用散点图，字段与指标清单用表格。图表编号见工具结果里的 figure.id，不要自己编造编号或数值，也不要为凑图对不适合相加的指标求和。
自主选择适合的工具：statistics 分析数值分布，group_by 分析分类频次或分类数值，trend 分析时间变化，correlate 比较两列数值的成对关系。纯文本表也可做频次与完整性分析。
...
最终报告是一篇中文分析文章：一个一级标题，若干二级标题，每个小节用连贯的正文段落展开，不要用项目符号罗列结论，细节可以放进 Markdown 表格。
把图表插进正文：在需要展示的位置单独一行写 [[fig-1]]（编号取工具结果里的 figure.id），每张图最多引用一次，尽量让每张图都出现在相关段落之后；没有被引用的图会自动附在文章末尾。
所有数值必须有本次工具结果依据；工具无法支持的问题直接说明。简洁，不编造。`;
